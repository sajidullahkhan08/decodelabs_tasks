...
const { transformCurrentWeather, kelvinToCelsius } = require('../../controllers/weatherController');
...
describe('Weather API', () => {
  ...
  describe('GET /api/weather/current', () => {
    ...
    it('should handle city not found', async () => {
      ...
      const res = await request(app).get('/api/weather/current').query({ city: 'InvalidCityXYZ' });
  ...
  describe('POST /api/weather/multiple', () => {
    it('should fetch weather for multiple cities', async () => {
      axios.get.mockResolvedValue({ data: mockWeatherResponse });
    ...
    it('should reject empty cities array', async () => {
      const res = await request(app)
        .post('/api/weather/multiple')
        .set('Authorization', `Bearer ${token}`)
        .send({ cities: [] });
